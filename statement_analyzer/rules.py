...
def check_large_round_amounts(transactions: list[Transaction], config: dict[str, Any]) -> list[Finding]:
    ...
    for item in transactions:
        ...
        is_round_amount = round_base > 0 and amount % round_base == 0
        if amount >= min_amount and is_round_amount:
            findings.append(
                Finding(
...
def build_metrics(transactions: list[Transaction]) -> dict[str, Any]:
    ...
    return {
        ...
        "monthly": {month: {key: str(value) for key, value in values.items()} for month, values in sorted(monthly.items())},
        "top_counterparties": [
